defmodule DeployEx.PrivManifestTest do
  ...
  setup do
    tmp_dir = Path.join(System.tmp_dir!(), "deploy_ex_manifest_test_#{System.unique_integer([:positive])}")
  ...
  describe "write/2 and read/1" do
    test "round-trips manifest through file", %{tmp_dir: tmp_dir} do
      ...
      assert {:ok, "sha256:abc123"} === PrivManifest.base_hash(read_manifest, "terraform/ec2.tf.eex")
    end
