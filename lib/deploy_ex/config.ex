defmodule DeployEx.Config do
  ...
  def aws_log_bucket, do: Application.get_env(@app, :aws_log_bucket) || "#{String.replace(DeployExHelpers.underscored_project_name(), "_", "-")}-backend-logs-#{env()}"
  def aws_log_region, do: Application.get_env(@app, :aws_log_region) || "us-west-2"

  def aws_release_bucket, do: Application.get_env(@app, :aws_release_bucket) || "#{String.replace(DeployExHelpers.underscored_project_name(), "_", "-")}-elixir-deploys-#{env()}"
