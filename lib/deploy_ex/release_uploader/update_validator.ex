defmodule DeployEx.ReleaseUploader.UpdateValidator do
  ...
  defp load_file_diffs(states) do
    ...
    with {:ok, file_diffs} <- file_diffs_between(from_to_shas) do
      case Enum.reject(file_diffs, fn
        {_sha_tuple, []} -> true
        {_sha_tuple, _} -> false
      end) do
        [] -> {:error, ErrorMessage.not_found("no file difs found for any shas")}
        file_diffs -> {:ok, file_diffs}
      end
    end
  end
  ...
  defp release_has_code_changes?(%DeployEx.ReleaseUploader.State{
    ...
    file_diffs = Keyword.get(file_diffs_by_sha_tuple, {current_sha, last_sha}, [])

    Enum.any?(file_diffs) and Enum.any?(file_diffs, &file_part_of_app(&1, app_name))
  end

  defp file_part_of_app({_sha_tuple, diff_file}, app_name) do
    diff_file =~ ~r/^#{app_name}\//
  end
  ...
  defp release_has_dep_changes?(%DeployEx.ReleaseUploader.State{
    ...
    dep_changes = Keyword.get(dep_changes_by_sha_tuple, {current_sha, last_sha})
