defmodule DeployEx.Grafana do
  ...
  @type install_result ::
          {:ok, %{title: String.t(), uid: String.t(), url: String.t()}}
    ...
    if opts[:grafana_ip] do
      ...
      with {:ok, instances} <- DeployEx.AwsMachine.fetch_instances_by_tag("Group", resource_group) do
        ...
        if is_nil(grafana_instance) do
          {:error,
           ErrorMessage.not_found("no running Grafana node found with MonitoringKey 'grafana_ui'")}
        else
  ...
  @spec download_dashboard(pos_integer() | String.t()) ::
    ...
    case Req.get(url) do
      ...
      {:ok, %Req.Response{status: 200, body: body}} when is_binary(body) ->
        case Jason.decode(body) do
          {:ok, decoded} ->
            {:ok, prepare_dashboard_for_import(decoded)}

          {:error, _} ->
            {:error, ErrorMessage.bad_request("failed to parse dashboard JSON from grafana.com")}
        end

      {:ok, %Req.Response{status: status}} ->
        {:error,
         ErrorMessage.bad_gateway(
           "grafana.com returned status #{status} for dashboard #{grafana_id}"
         )}

      {:error, exception} ->
        {:error,
         ErrorMessage.failed_dependency("failed to download dashboard from grafana.com", %{
  ...
  defp post_dashboard(dashboard_json, local_port, opts) do
    ...
    case Req.post(url, json: body, auth: {:basic, "#{user}:#{password}"}) do
      ...
      {:error, exception} ->
        {:error,
         ErrorMessage.failed_dependency("failed to connect to Grafana API", %{
           error: Exception.message(exception)
         })}
    end
  end
