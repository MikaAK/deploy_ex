defmodule DeployEx.SSH do
  ...
  def run_command(ip, port \\ 22, pem_file_path, command) do
    case connect_to_ssh(ip, port, pem_file_path) do
      {:error, reason} ->
        {:error, ErrorMessage.bad_gateway("couldn't connect over ssh: #{reason}")}
  ...
  defp run_session_command(conn, command) do
    ...
    tap(
      case :ssh_connection.exec(conn, channel, "sudo -u root #{command}", :timer.minutes(2)) do
        :success -> receive_message()
        {:error, reason} -> {:error, ErrorMessage.failed_dependency("ssh command failed", reason)}
      end,
      fn _ -> :ssh_connection.close(conn, channel) end
    )
  end
  ...
  def receive_message(return_message \\ "") do
    receive do
      {:ssh_cm, _pid, {:data, _cid, 1, data}} ->
        receive_message(return_message <> data)
      ...
      {:ssh_cm, _pid, {:exit_status, _cid, code}} ->
        {:error,
         ErrorMessage.failed_dependency("return from command failed with code #{code}", %{
    ...
    after
      :timer.seconds(30) ->
        {:error, ErrorMessage.failed_dependency("no return from command after 30 seconds")}
    end
  end
  ...
  def find_available_port do
    case :gen_tcp.listen(0, []) do
      ...
      {:error, reason} ->
        {:error,
         ErrorMessage.internal_server_error("Failed to find available port", %{reason: reason})}
    end
  end
