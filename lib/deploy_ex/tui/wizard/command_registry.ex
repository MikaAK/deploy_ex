defmodule DeployEx.TUI.Wizard.CommandRegistry do
  ...
  defp build_commands do
    [
      ...
      %{
        ...
        category: "DeployEx",
        inputs: [
          input(:auto_approve, "Auto-approve Terraform", :boolean,
            description: "Skip Terraform plan confirmation"
          ),
          input(:skip_deploy, "Skip deploy", :boolean,
            description: "Skip application deployment after server setup"
          ),
          input(:skip_setup, "Skip setup wait", :boolean,
            description: "Skip waiting period between infra creation and setup"
          )
        ]
      },
      %{
      ...
      %{
        ...
        inputs: [
          ...
          input(:index, "Instance index", :integer,
            description: "Connect to a specific instance (0-based)"
          ),
      ...
      %{
        ...
        inputs: [
          ...
          input(:except, "Except app(s)", :string,
            description: "Comma-separated app names to skip"
          ),
  ...
  def search(query) do
    ...
    |> Enum.map(fn cmd ->
      ...
      shortdoc_score =
        if shortdoc !== "",
          do: String.jaro_distance(query_lower, String.downcase(shortdoc)),
          else: 0.0

      category_score = String.jaro_distance(query_lower, String.downcase(cmd.category))

      contains_bonus =
        if String.contains?(String.downcase(cmd.task), query_lower) or
             String.contains?(String.downcase(shortdoc), query_lower),
           do: 0.2,
           else: 0.0
