defmodule Mix.Tasks.Terraform.CreateEbsSnapshot do
  ...
  def run(args) do
    ...
    with :ok <- DeployExHelpers.check_in_umbrella(),
         ...
         {:ok, instance_ips} <-
           AwsMachine.find_instance_ips(DeployExHelpers.project_name(), app_name, machine_opts),
         ...
         {:ok, snapshots} <-
           create_snapshots_for_volumes(region, filtered_volumes, app_name, opts) do
      Mix.shell().info([
        :green,
      ...
      Enum.each(snapshots, fn snapshot ->
        ...
        volume_type_info =
          if snapshot["volume_type"], do: " - #{snapshot["volume_type"]}", else: ""
    ...
    filtered_volumes =
      Enum.filter(volumes, fn volume ->
        ...
        case volume_type do
          ...
          # Include other volumes only if include_root is true
          _ -> include_root
        end
      end)
    ...
    case filtered_volumes do
      ...
      volumes ->
        ...
        Mix.shell().info([
          ...
          :reset,
          :green,
          " volume(s) to snapshot: ",
          :cyan,
          Enum.join(volume_types, ", "),
          :reset
        ])
  ...
  defp parse_volumes_response(body) do
    case XmlToMap.naive_map(body) do
      %{"DescribeVolumesResponse" => %{"volumeSet" => %{"item" => volumes}}}
      when is_list(volumes) ->
        {:ok, volumes}
  ...
  defp create_snapshot_for_volume(region, volume, description) do
    ...
    snapshot_description =
      "#{description} (Volume: #{volume_id}#{if device, do: " - Device: #{device}", else: ""}#{if volume_type, do: " - #{volume_type}", else: ""})"
  ...
  defp is_root_volume?(device, volume) do
    cond do
      ...
      volume["size"] && String.to_integer(volume["size"]) <= 50 ->
        true

      # Check if it's the boot volume
      volume["attachmentSet"]["item"]["deleteOnTermination"] === "true" ->
        true
