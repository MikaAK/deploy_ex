  ...
  defp report_orphans(s3_orphans, instance_orphans, _opts) do
    ...
    unless Enum.empty?(instance_orphans) do
      Mix.shell().info([
        ...
        "Terminate ",
        :reset,
        "#{length(instance_orphans)} orphaned instance(s)"
      ])
    end
  end
      ...
      case DeployEx.QaNode.delete_qa_state(state, opts) do
        :ok ->
          unless opts[:quiet] do
            Mix.shell().info([
              :green,
              "  ✓ Deleted S3 state for #{app_name} (#{state.instance_id})"
            ])
          end

        {:error, error} ->
          Mix.shell().error(
            "  ✗ Failed to delete S3 state for #{app_name}: #{ErrorMessage.to_string(error)}"
          )
      end
    end)
  end
  ...
  defp cleanup_instance_orphans(orphans, opts) do
    Enum.each(orphans, fn instance ->
      ...
      case DeployEx.QaNode.terminate_instance(instance_id, opts) do
        ...
        {:error, error} ->
          Mix.shell().error(
            "  ✗ Failed to terminate #{instance_id}: #{ErrorMessage.to_string(error)}"
          )
      end
    end)
  end
