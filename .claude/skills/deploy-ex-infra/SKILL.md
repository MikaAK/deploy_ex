...
description: "Use when managing AWS infrastructure through deploy_ex — generating Terraform .tf files from templates, running terraform plan/apply/init/refresh, building Ansible playbooks and config, setting up servers with ansible.setup, database dump and restore via SSH tunnel, EBS snapshot management, Terraform state bucket and lock table setup, configuring deploy_ex settings (aws_region, buckets, AMI, terraform_backend), extracting PEM keys, showing database passwords, upgrading priv templates, installing GitHub Actions, or tearing down infrastructure. This is for INFRASTRUCTURE PROVISIONING and CONFIGURATION, not for deploying app releases or SSH-ing into running instances. Triggers on: terraform build/plan/apply/drop, ansible build, generate .tf files, dump database, restore database, EBS snapshot, state bucket, configure deploy_ex, setup from scratch, replace EC2 instances, terraform state out of sync, upgrade templates, install github action, show password, generate pem."
---
